...
export class RewriterManager {
  ...
  getCodeSnippet(options: any, input: string): string {
    const tone = options.tone ? `tone: "${options.tone}",` : '';
    const length = options.length ? `length: "${options.length}",` : '';
...
const result = await rewriter.rewrite("${escapedInput}");
console.log(result);
`;
  }
  ...
  async getStatus(): Promise<ApiStatusResult> {
    ...
    const hasGlobal = 'Rewriter' in window;
    checks.push({
      titleHtml: 'Global Object (<code>window.Rewriter</code>)',
      success: hasGlobal
    });
    ...
    if (!hasGlobal) {
      errorHtml = `
        ...
        <div class="text-xs text-red-100">
            <p class="mb-2">The <code>window.Rewriter</code> object was not found. This shouldn't happen, unless you are using an unsupported version of Chrome. Please contact us. You can also try these steps:</p>
            <ul class="list-disc pl-4 space-y-1">
                <li>Navigate to <code>chrome://flags</code>.</li>
                <li>Search for <strong>"Rewriter API"</strong> and enable it.</li>
      ...
      return {
        ...
        message: 'Rewriter API is not supported.',
        checks,
        errorHtml
      };
    }
    ...
    try {
      ...
      if (status === ApiStatus.AVAILABLE || status === 'readily' as any) {
          ...
          errorHtml = `
            ...
            <div class="text-xs text-orange-100">
                <p class="mb-2">The Rewriter API model needs to be downloaded before it can be used.</p>
                <ul class="list-disc pl-4 space-y-1">
          ...
          message = 'Rewriter API unavailable.';
          status = ApiStatus.UNAVAILABLE;
          errorHtml = `
            <h4 class="text-xs font-bold text-red-200 uppercase mb-2"><i class="fa-solid fa-ban mr-1"></i> API Unavailable</h4>
            <div class="text-xs text-red-100">
                <p class="mb-2">The Rewriter API is currently unavailable for reasons unknown. This might be a temporary issue or indicate a problem with your browser configuration.</p>
                <ul class="list-disc pl-4 space-y-1">
      ...
      errorHtml = `
        ...
        <div class="text-xs text-red-100">
            <p class="mb-2">An error occurred while checking the Rewriter API availability: <code>${e.message}</code></p>
            <ul class="list-disc pl-4 space-y-1">
                <li>Please file a bug.</li>
            </ul>
        </div>
