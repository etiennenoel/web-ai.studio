...
export class TranslatorManager {
  ...
  getCodeSnippet(source: string, target: string, input: string): string {
...
const result = await translator.translate("${escapedInput}");
console.log(result);`;
  }
  ...
  async getStatus(): Promise<ApiStatusResult> {
    ...
    // Check 1: Global Object
    const hasGlobal = 'Translator' in window;
    checks.push({
      titleHtml: 'Global Object (<code>window.Translator</code>)',
      success: hasGlobal
    });
    ...
    if (!hasGlobal) {
      errorHtml = `
        ...
        <div class="text-xs text-red-100">
            <p class="mb-2">The <code>window.Translator</code> object was not found. This shouldn't happen, unless you are using an unsupported version of Chrome. Please contact us. You can also try these steps:</p>
            <ul class="list-disc pl-4 space-y-1">
                <li>Navigate to <code>chrome://flags</code>.</li>
                <li>Search for <strong>"Translation API"</strong> and enable it.</li>
      ...
      return {
        ...
        message: 'Translator API is not supported in this browser.',
        checks,
        errorHtml
      };
    }
    ...
    try {
      ...
      // Map legacy/alternative statuses if necessary, though 'available' is standard now
      if (status === ApiStatus.AVAILABLE || status === 'readily' as any) {
          ...
          errorHtml = `
            ...
            <div class="text-xs text-orange-100">
                <p class="mb-2">The Translator API model needs to be downloaded before it can be used.</p>
                <ul class="list-disc pl-4 space-y-1">
          ...
          message = 'Translator API is unavailable.';
          status = ApiStatus.UNAVAILABLE;
          errorHtml = `
            <h4 class="text-xs font-bold text-red-200 uppercase mb-2"><i class="fa-solid fa-ban mr-1"></i> API Unavailable</h4>
            <div class="text-xs text-red-100">
                <p class="mb-2">The Translator API is currently unavailable for reasons unknown. This might be a temporary issue or indicate a problem with your browser configuration.</p>
                <ul class="list-disc pl-4 space-y-1">
      ...
      message = `Error checking availability: ${e.message}`;
      checks.push({
        titleHtml: `Availability Check Failed: ${e.message}`,
        success: false
      });
      errorHtml = `
        <h4 class="text-xs font-bold text-red-200 uppercase mb-2"><i class="fa-solid fa-exclamation-triangle mr-1"></i> Error Details</h4>
        <div class="text-xs text-red-100">
            <p class="mb-2">An error occurred while checking the Translator API availability: <code>${e.message}</code></p>
            <ul class="list-disc pl-4 space-y-1">
                <li>Please file a bug.</li>
            </ul>
        </div>
