...
export class HistoryComponent implements OnInit {
      ...
      this.rawItems = [
        {
          ...
          timestamps: { create: Date.now() - 61000, completed: Date.now() - 60500 },
        ...
        },
        {
          ...
          timestamps: {
            execute: Date.now() - 60000,
            first_token: Date.now() - 59000,
            completed: Date.now() - 58000,
          },
          args: ['Write a poem'],
        },
        {
          ...
          timestamps: { create: Date.now() - 3601000, completed: Date.now() - 3600500 },
        },
        {
          id: '4',
          sessionId: 's2',
          ...
          timestamps: { execute: Date.now() - 3600000, completed: Date.now() - 3595000 },
          args: ['Long text...'],
        },
      ];
      this.isLoading = false;
  ...
  hasText(group: SessionGroup): boolean {
    // Virtually all sessions involve text in some way (prompts, options, setup)
    return true;
  }

  hasImage(group: SessionGroup): boolean {
    return group.createItem?.hasImage || false || group.methodItems.some((item) => item.hasImage);
  }

  hasAudio(group: SessionGroup): boolean {
    return group.createItem?.hasAudio || false || group.methodItems.some((item) => item.hasAudio);
  }
