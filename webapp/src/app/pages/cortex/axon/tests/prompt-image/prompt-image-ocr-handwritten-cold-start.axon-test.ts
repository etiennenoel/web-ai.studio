...
@Injectable()
export class PromptImageOcrHandwrittenColdStartAxonTest implements AxonTestInterface {
  id: AxonTestId = AxonTestId.PromptImageOcrHandwrittenColdStart;
  ...
  results: AxonTestResultInterface = {
    ...
    input: "Extract the handwritten text from the provided image canvas.",
  ...
  async run(): Promise<AxonTestResultInterface> {
    ...
    const testWord = "HelloWorld";
    const image = await ImageTestUtils.createTextImage(testWord, "italic 40px 'Comic Sans MS', cursive");
