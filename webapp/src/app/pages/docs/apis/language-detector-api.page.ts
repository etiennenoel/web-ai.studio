...
@Component({
  selector: 'app-language-detector-api-docs',
  template: `
      ...
      <div class="w-full p-6 md:p-12 pb-32 font-sans">
        ...
        <div class="mb-10">
          <nav class="text-sm font-medium text-slate-500 dark:text-slate-400 mb-6 flex items-center space-x-2">
            ...
            <span class="text-slate-900 dark:text-slate-200">Language Detector</span>
          </nav>
          ...
          <div class="mb-8">
            <div class="flex items-center gap-3 mb-2">
                ...
                <h1 class="text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight">Language Detector API</h1>
              </div>
          ...
          <p class="text-base text-slate-600 dark:text-slate-400 leading-relaxed max-w-4xl">
            The Language Detector API identifies the language of a given text. It returns a ranked array of probable BCP 47 language tags along with a confidence score. It is highly useful as a precursor to the Translator API when dealing with arbitrary user input.
          </p>
        </div>
        ...
        <div class="mb-12 max-w-4xl">
          ...
          <p class="text-slate-600 dark:text-slate-400 text-sm leading-relaxed mb-4">
            The API is exposed via the <code class="font-mono text-xs bg-slate-100 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-slate-700 dark:text-slate-300 px-2 py-1 rounded">LanguageDetector</code> global object.
          </p>
        </div>
        ...
        <div class="mb-12 max-w-4xl">
          ...
          <div class="space-y-8">
            ...
            <div class="border border-slate-200 dark:border-zinc-800 rounded-xl overflow-hidden">
              <div class="bg-slate-50 dark:bg-zinc-900/50 px-5 py-3 border-b border-slate-200 dark:border-zinc-800">
                <h3 class="font-mono text-sm font-bold text-slate-900 dark:text-slate-200">
                  LanguageDetector.create(<span class="text-slate-500 font-normal">options?</span>)
                </h3>
              </div>
              <div class="p-5">
                ...
                <ul class="list-disc pl-5 text-sm text-slate-600 dark:text-slate-400 space-y-2">
                  <li><code>expectedInputLanguages</code> (array): An array of BCP 47 strings to preload models for specific expected inputs, causing the promise to reject if unsupported.</li>
                </ul>
              </div>
            </div>
            ...
            <div class="border border-slate-200 dark:border-zinc-800 rounded-xl overflow-hidden">
              <div class="bg-slate-50 dark:bg-zinc-900/50 px-5 py-3 border-b border-slate-200 dark:border-zinc-800">
                <h3 class="font-mono text-sm font-bold text-slate-900 dark:text-slate-200">
                  detector.detect(<span class="text-slate-500 font-normal">text</span>)
                </h3>
              </div>
              <div class="p-5">
                <p class="text-slate-600 dark:text-slate-400 text-sm mb-4">
                  Analyzes the text and resolves to an array of objects.
                </p>
                <h4 class="text-xs font-bold text-slate-900 dark:text-slate-300 mb-2 uppercase tracking-wider">Returns Array of:</h4>
                <ul class="list-disc pl-5 text-sm text-slate-600 dark:text-slate-400 space-y-2">
                  <li><code>detectedLanguage</code> (string): The BCP 47 tag. (Or <code>"und"</code> for undetermined probability).</li>
                  <li><code>confidence</code> (number): A value between 0.0 and 1.0.</li>
                </ul>
              </div>
            </div>
        ...
        <div class="mt-16 pt-8 border-t border-slate-200 dark:border-zinc-800 flex justify-between items-center">
          <a routerLink="/docs/translator" class="!no-underline group flex flex-col items-start px-4 py-3 rounded-xl hover:bg-slate-50 dark:hover:bg-zinc-800/50 transition-colors border border-transparent hover:border-slate-200 dark:hover:border-zinc-700">
          ...
          <a routerLink="/docs/proofreader" class="!no-underline group flex flex-col items-end px-4 py-3 rounded-xl hover:bg-slate-50 dark:hover:bg-zinc-800/50 transition-colors border border-transparent hover:border-slate-200 dark:hover:border-zinc-700 text-right">
