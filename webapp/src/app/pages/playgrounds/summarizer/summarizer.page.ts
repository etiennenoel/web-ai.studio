...
@Component({
  selector: 'app-summarizer-playground',
  templateUrl: './summarizer.page.html',
  standalone: false,
  host: { class: 'block w-full h-full overflow-y-auto bg-[#ffffff] dark:bg-[#121212]' }
})
export class SummarizerPlaygroundPage implements OnInit, OnDestroy {
  ...
  initForm() {
    this.playgroundForm = this.fb.group({
      ...
      // Execution Options
      promptInput: ['Artificial intelligence is intelligence demonstrated by machines, as opposed to the natural intelligence displayed by animals including humans. AI research has been defined as the field of study of intelligent agents.', Validators.required],
      context: [''],
  ...
  getSummarizer() {
    return (window as any).ai?.summarizer || (window as any).Summarizer;
  }
  ...
  getCreateOptions() {
    ...
    if (val.type) options.type = val.type;
    if (val.format) options.format = val.format;
    if (val.length) options.length = val.length;
    if (val.preference && val.preference !== 'auto') options.preference = val.preference;
  ...
  async runPrompt(isStreaming: boolean) {
    ...
    try {
      ...
      if (isStreaming) {
        const stream = this.session.summarizeStreaming(val.promptInput, options);
        ...
        const result = await this.session.summarize(val.promptInput, options);
        this.fullOutput = result;
      }
      
    } catch (e: any) {
      this.errorMessage = e.message || 'Error during summarization';
    } finally {
  ...
  updateGeneratedCode() {
    ...
    let code = `// Summarizer API Configuration\n\n`;
    code += `const options = {\n`;
    if (val.type) code += `  type: "${val.type}",\n`;
    if (val.format) code += `  format: "${val.format}",\n`;
    if (val.length) code += `  length: "${val.length}",\n`;
    if (val.preference && val.preference !== 'auto') code += `  preference: "${val.preference}",\n`;
    ...
    code += `const availability = await Summarizer.availability(options);\n\n`;
    ...
    code += `const session = await Summarizer.create(options);\n\n`;
    
    code += `const summarizeOptions = {};\n`;
    if (val.context) code += `summarizeOptions.context = ${JSON.stringify(val.context)};\n`;
    if (val.useAbortSignal) code += `summarizeOptions.signal = new AbortController().signal;\n\n`;
    
    code += `// Streaming\n`;
    code += `const stream = session.summarizeStreaming(${JSON.stringify(val.promptInput)}, summarizeOptions);\n`;
    code += `for await (const chunk of stream) {\n  console.log(chunk);\n}\n\n`;
    
    code += `// Non-Streaming\n`;
    code += `const result = await session.summarize(${JSON.stringify(val.promptInput)}, summarizeOptions);\n\n`;
    
    code += `const tokens = await session.measureInputUsage(${JSON.stringify(val.promptInput)}, summarizeOptions);\n`;
