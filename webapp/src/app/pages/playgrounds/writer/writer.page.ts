...
@Component({
  selector: 'app-writer-playground',
  templateUrl: './writer.page.html',
  standalone: false,
  host: { class: 'flex flex-col h-full w-full min-h-0' }
})
export class WriterPlaygroundPage implements OnInit, OnDestroy {
  ...
  initForm() {
    this.playgroundForm = this.fb.group({
      ...
      // Execution Options
      promptInput: ['An email to my boss asking for a deadline extension.', Validators.required],
      context: [''],
  ...
  getWriter() {
    return (window as any).ai?.writer || (window as any).Writer;
  }
  ...
  getCreateOptions() {
    ...
    if (val.tone) options.tone = val.tone;
  ...
  async runPrompt(isStreaming: boolean) {
    ...
    try {
      ...
      if (isStreaming) {
        const stream = this.session.writeStreaming(val.promptInput, options);
        ...
        const result = await this.session.write(val.promptInput, options);
  ...
  updateGeneratedCode() {
    ...
    let code = `// Writer API Configuration\n\n`;
    code += `const options = {\n`;
    if (val.tone) code += `  tone: "${val.tone}",\n`;
    ...
    code += `const availability = await Writer.availability(options);\n\n`;
    ...
    code += `const session = await Writer.create(options);\n\n`;
    
    code += `const writeOptions = {};\n`;
    if (val.context) code += `writeOptions.context = ${JSON.stringify(val.context)};\n`;
    if (val.useAbortSignal) code += `writeOptions.signal = new AbortController().signal;\n\n`;
    
    code += `// Streaming\n`;
    code += `const stream = session.writeStreaming(${JSON.stringify(val.promptInput)}, writeOptions);\n`;
    code += `for await (const chunk of stream) {\n  console.log(chunk);\n}\n\n`;
    
    code += `// Non-Streaming\n`;
    code += `const result = await session.write(${JSON.stringify(val.promptInput)}, writeOptions);\n\n`;
    
    code += `const tokens = await session.measureInputUsage(${JSON.stringify(val.promptInput)}, writeOptions);\n`;
